...
print(
    standart_deviations(num_friends_three),
...
def sum_least_squares(alfa, beta, y_mean):
    return sum(
        [(predict(alfa, beta, y_predict) - y_mean) ** 2 for y_predict in daily_minutes]
    )
...
def r_square(alfa, beta):
    ...
    print(total_sum_of_squares(mean(daily_minutes)), " total_sum_of_squares")
    return 1 - (
        sum_least_squares(alfa, beta, mean(daily_minutes))
