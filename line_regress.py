...
def line_regres(intercept):
    ...
    while True:
        for data_weight in datas:
            ...
            print(
                height, " height ;", predict_height, " predict_height"
            )  # если predict_height убывает то ф-я убывающая
        ...
        print("")
        print(
            " Второй вариант расчёта сумм квадратов отклонений residuals через listcomprehension"
        )
        print("")
        data_intercept = [
            0,
            ...
            0.75,
            1,
        ]  # значения пересечения линии с axis height
        sum_square_resuduals = [
            (data_weight[1] - (intercept + (0.64 * data_weight[0]))) ** 2
            for intercept in data_intercept
            for data_weight in datas
        ]
