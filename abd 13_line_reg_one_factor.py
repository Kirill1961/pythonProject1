...
def minimz(x_data, y_data, l_r):
    index_random_for_x_y = random.choice(
        [index_x for index_x, index_y in enumerate(y_data)]
    )
    ...
    while quantity_iteration_for_g_d < 5000:
        ...
        print(
            round(theta_0[1], 3), " theta_0_β before переопределения"
        )  # round - ограничение знаков после запятой

        theta_0 = [
            theta_0[k] - step_size_for_a_b[k] for k in range(2)
        ]  # индекс k - выбор для ᶐ или β
        print(round(theta_0[0], 5), " theta_0_ᶐ after переопределения")
        print(round(theta_0[1], 5), "theta_0_β after переопределения")

        quotient_no_improvement = [
            round(theta_0[1], 3) == round(theta_n[1], 3) for _ in range(3)
        ]  # 3-x кратная проверка равенства theta_0 и theta_n
        print(quotient_no_improvement, " quotient_no_improvement")
        beta = theta_0[1]

        if (
            s_e_g_a_b[0] < 0.000001 or s_e_g_a_b[1] < 0.000001
        ):  # условие gпо минимальным градиентам
...
def sgd_for_a_b(x_data, y_data, l):
    ...
    a_b_result = np.array(
        res
    )  #  преобразование  списка в матрицу для сложения поэлементно
    sum_sgd_a_b = a_b_result.sum(axis=0).tolist()  # сложение матрицы по столбцам
    alfa_sgd, beta_sgd = sum_sgd_a_b[0] / len(y_data), sum_sgd_a_b[1] / len(
        x_data
    )  # среднее коэфф ᶐ и β
    ...
    print(
        alfa_sgd,
        " - alfa_sgd градиент свободного аргумента д/быть близок к нулю ",
        beta_sgd,
        " - beta_sgd",
    )
