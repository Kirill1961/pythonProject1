...
)  # fetch_california_housing() - функция из sklearn, в виде словаря
data = pd.DataFrame(
    california.data, columns=california.feature_names
)  # DataFrame - массив
...
model_quality_ = [
    (i, j) for i, j in zip(california.feature_names, coef)
]  # Качество модели
...
intercept = (
    model.intercept_
)  # значение свободного параметра, те значение целевой переменной если бы все Х были = 0
