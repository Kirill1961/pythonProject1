...
def e_r(x_data, y_data, theta_n):  # residual, theta_n - это вектор β
    ...
    print(
        list(map(lambda w: [y_data - v * w for v in x_data], theta_n))[0][1:],
        " e_r без intersept",
    )
    return list(map(lambda w: [y_data - v * w for v in x_data], theta_n))[
        0
    ]  # 2-й вариант вычисления
...
def minimz_multi_value(l_r):
    ...
    while numer_iteration < 1:  # quantity_ index
        ...
        beta_for_theta_n = (
            beta_initial_for_theta_n  # переопределение вектора β после инициализации
        )
        ...
        if beta_for_theta_n == min_beta_for_theta_n:
            ...
            while quantity_iteration_for_g_d < 10:
                ...
                print(
                    "\t" * 3,
                    quantity_iteration_for_g_d,
                    " quantity_iteration_for_g_d",
                    "\n",
                )

                step_size_for_betas = [
                    s * l_r for s in s_e_g_multi_value(*data_x_y, beta_for_theta_n)
                ][
                    1:
                ]  # без βₒ
