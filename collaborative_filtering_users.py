...
def make_user_interest_vector(user_interests):
    return [
        1 if interest in user_interests else 0 for interest in unique_interests
    ]  # вектор интересов
...
def most_similar_users_to(user_id):
    ...
    pairs = [
        (other_user_id, similaraty)
        for other_user_id, similaraty in enumerate(user_similarities[0])
        if user_id != other_user_id and similaraty > 0
    ]  # отсев текущего юзера и подобия = 0
...
def user_based_suggetions(user_id, include_current_interests=False):
    ...
    for other_user_id, similarity in most_similar_users_to(
        user_id
    ):  # вытаскиваем id друга + подобие
        for interest in users_interests[other_user_id]:  # вытаскиваем интересы друзей
            suggetions[
                interest
            ] += similarity  # пишем интересы друзей в key словаря : value - сумма подобий
    suggetions = sorted(
        suggetions.items(), key=lambda i: i[1], reverse=True
    )  # сортировка по подобию, [1]index кортежа
    ...
    if include_current_interests:
        ...
        return [
            (suggetion, wight)
            for suggetion, wight in suggetions
            if suggetion not in users_interests[user_id]
