...
print(
    d.split(),
    "split() - делит по умолчанию по пробелам если их нет,то в списке будет целиком строка,одно значение",
)

print(
    d.split(sep=","),
    '  sep="," - доп параметр который указывает по какому символу делить строку ',
)
...
print(
    b.split(sep=","),
    ' если sep="," по запятой а в строке её нет то в списке будет целиком строка',
)


# maxsplit=1 указывает сколько раз будет разделена строка
print(
    d.split(sep=",", maxsplit=1),
    " maxsplit=1 указывает сколько раз будет разделена строка",
)
...
c = [
    "asdf, poiu, poiu, dfghrtyu, rtyu, asdfpoiu, asdf, asdf, qwert, rewq, asdf, asdddd"
]

print(
    "".join(c).split(),
    " преобразование списка из 1й строки в список строчных объектов ",
    "\n",
)
...
# printing result
print(
    "The String of list is : " + str(res),
    " преобразование списка списков со строчными ОБ в множество строчных ОБ",
    "\n",
)
...
new_text = text.replace(
    "JavaScript", "Python", 2
)  # 1й арг - что меняем, 2й арг - на что меняем, 2 - кол-во замен
