...
counts = defaultdict(
    lambda: [1, 1]
)  # lambda передаёт в созданный словарь величину [1, 1]
counts["asdf"].append(555)
print(
    counts,
    " величине [1, 1] в созданном словаре присвоили key - asdf и добавили величину 555",
)
...
z.append(555)
print(
    x(), " lambda Без аргументов"
)  # если " х " вызываем со скобками ты выводится содержимое тела ф-ции
print(z, " append(555)")
...
print(
    sort_lamb, " в переменную ite_m поочерёдно передаются пары из dict в виде кортежа"
)
...
print(
    [i for i in filter(lambda row: not row[1], f)][:],
    " фильтрация булевых типов данных  True / False",
)
print(
    [i for i in filter(lambda row: row, f)][-4:],
    " та же конструкция но весь список f",
    "\n",
)
...
result_warlus = (
    ...
    w**2,
    w**3,
)  # func() вызывается один раз и записывается в переменную w
...
print(
    result_lambda([v := func(200), v**2, v**3]),
    " result_lambda со своим аргументом",
    "\n",
)
...
]  # прокручиваем data через func(x), func() вызывается три раза

res_y_func = [
    y for x in data if (y := func(x))
]  # func() вызывается один раз и записывается в переменную y
...
print(
    [r for r in filter(lambda r: r is False, bo_ol)],
    " фильтруем из списка bo_ol False",
    "\n",
)
print(
    [r for r in filter(lambda r: r is True, bo_ol)],
    " фильтруем из списка bo_ol True",
    "\n",
)
...
print(
    list(filter(lambda x: type(x) == list, data_structure))[0],
    " Фильтрация по структуре данных, здесь" " отделяем list от всего остального",
    "\n",
)
...
b = [2, 2, 2, 2]
print(
    list(map(lambda x_f, e_r_f: e_r_f * x_f, c, b)),
    " Поиндексное действие П Р О С Т О",
    "\n",
)
...
print(
    list(
        map(
            lambda enum_and_y, data: [d * y_[enum_and_y[0]] for d in data],
            enumerate(y_),
            x_,
        )
    ),
...
print(
    ar_reduc_1,
    " reduce уменьшает размерность матрицы суммируя ОБ в матрице, сумма ОБ не меняется = 78",
    "\n",
)
...
print(
    l,
    " Использование цепочки методов в lambda функции, здесь применены l.pop(),l.append(x), l.insert(0, 100)",
    "\n",
)
...
print(
    ...
    "min значение в списке, проверка поиндексно",
    "\n",
)
