...
def split_data(
    data, prob
):  # prob  число определяющее долю данных для обучения, для теста останется 1 - prob данных
    results = [], []  # results - кортеж из пустых списков
    for row in data:
        results[0 if random.random() < prob else 1].append(
            row
        )  # индексы кортежа results для заполнения 2х списков data
...
def train_test_split(x, y, test_pct):  # test_pct - процент данных для test
    ...
    train, test = split_data(
        data, 1 - test_pct
    )  # "1 - test_pct" - кол-во данных для обучения
    print(
        train,
        "тренировочная выборка",
        test,
        " тестовая выборка соответствует заданному % test_pct",
    )
    x_train, y_train = zip(
        *train
    )  # в аргументе zip распаковщик " * ", это разъеденение кортежей
    x_test, y_test = zip(
        *test
    )  # если в аргументе " * " - zip совершает обратное действие-разъеденяет кортежи
