...
def conn_fre(x, y):
    ...
    print(
        " ", *[e for e in range(len(connect_friend))], sep="  "
    )  # распаковали список с помощью звезды " * "
...
print(
    ...
    g_matrix.shape,
    " - размерность по каждому измерению",
    "\n",
)
...
print(
    m[np.where(mx == 86)],
    " Определяем по ИНДЕКСУ значение в матрице m или вложенный список в L[L] - C",
)
...
print(
    a_full,
    " Генерация матрицы с одинаковыми значениями, np.full((1,3,3 - это shape),20 - значение в матрицу) ",
    "\n",
)
...
print(
    [[d[j][i] for i in range(3)] for j in range(3)],
    "Вытаскиваем из матрицы строки для действий",
    "\n",
)
print(
    [[c[j][i] for i in range(3)] for j in range(2)],
    "Вытаскиваем из матрицы строки для действий",
    "\n",
)
...
b = np.rollaxis(
    e, 2, 0
).shape  # Ось с индексом 2 это "5" перемещаем на место с индексом 0
print(
    np.ones((b)),
    " Перемещение осей при задании матрицы, 5 матриц 6 строки и 3 столбов ",
    "\n",
)
...
print(
    ...
    "np.piecewise Определим метки -1 если х<0 и +1 если х>0",
    "\n",
)
...
print(
    np.stack([e, u], axis=0),
    "np.stack - Объеденения 2х мерных массивов с одинаковым shape ",
    "\n",
)
...
print(
    list(zip([f, g])),
    "Объеденения 2х мерных массивов с разным shape list(zip(h, o)  :-)",
    "\n",
)
...
print(
    np.c_[h, o],
    "c_  ->  Объединение 2-х мерных массивов, с_ только с одинаковым shape, axis=1",
    "\n",
)
...
print(
    np.empty((3, 2, 2)),
    "np.empty Заполняется последним записанным array или slice",
    "\n",
)
...
print(
    [[j for j in a[i]] for i in range(5)], " Построчная итерация массива for+for", "\n"
)

# Разметка массива по признаку 1 - ОБ > 50 и 0 - ОБ < 50
print(
    np.array([[0 if j < 50 else 1 for j in a[i]] for i in range(5)]),
    "Разметка массива по признаку 1 - ОБ > 50 и 0 - ОБ < 50",
    "\n",
)
