...
def variance_for_correlation(x_y_dev):  # ДИСПЕРСИЯ Или variance
    ...
    print(
        "\t" * 7,
        variance_su_sq / (n - 1),
        " ДИСПЕРСИЯ или variance это сумма квадратов отклонений / n - 1",
    )
    return variance_su_sq / (
        n - 1
...
print(
    ...
    " >>>>>>>>>>>  std со значением дисперсии из  variance(x_)",
)
...
def covariance(x_, y_):
    ...
    print(
        x_bar,
        y_bar,
        '<<<<<<<<<<<<<<<<<<  Среднее д/ вариационных рядов " num_friends " и " daily_minutes "',
    )
...
num_friends_good = [
    num_friends[k] for k, i in enumerate(num_friends) if k != outlier_index
]
