...
def square(x_x, y_y, t):
    ...
    return (
        y_y - t * x_x
    ) ** 2  # Loss function целевая функция - квадратичное отклонение  / Loss function


def derivative(
    x_x, y_y, t
):  # производная целевой ф-ции Loss function по theta - (y_y - (t * x_x))**2
...
v_theta_for_sgd = (
    0.1 * v_theta if v_theta > 0 else v_theta * -1
)  # вектор параметров theta, * -1 это если random отриц.
...
def minimize_stohastic(target_fn, gradient_fn, x, y, theta_0, alfa_0=0.01):
    ...
    alfa = alfa_0
    min_theta, min_value = None, float(
        "inf"
    )  # определяем минимальные значения theta и value
    iterration_whith_no_improvement = 0
    iterration_whith_no_improvement_condition_for_value = 0
    while (
        iterration_whith_no_improvement < 100
        and iterration_whith_no_improvement_condition_for_value < 500
    ):
        value = sum(
            target_fn(x_i, y_i, theta) for x_i, y_i in data
        )  # Сумма квадратов residual (square)
        if (
            value < min_value
        ):  # условие если значение лежит на прямой те апроксимирована к прямой
            min_theta, min_value = theta, value
            print(" ")
            iterration_whith_no_improvement = 0
            iterration_whith_no_improvement_condition_for_value += 1
            print(
                "\t" * 6, "*", iterration_whith_no_improvement_condition_for_value, "*"
            )
            print(" ")
            alfa = alfa_0
        else:
            ...
            for x_i, y_i in in_random_order(data):
                ...
                gradient_i = gradient_fn(
                    x_i, y_i, theta
                )  # градиент Loss(target_fn) по  theta i-ое
                print(gradient_i, " gradient_i")
                print("\t" * 16, theta, " theta_previous ")
                theta = vector_subtract(
                    theta, scalar_multiplay(alfa, gradient_i)
                )  # вычисление следующ. theta
...
print(
    minimize_stohastic(
        square, derivative, x_for_sgd, y_for_sgd, v_theta_for_sgd, alfa_0=0.01
    ),
    " minimize_stohastic",
)
