...
class A:
    ...
    def __init__(self, x, y):
        ...
        self.x = self.y = 0
        if (
            A.class_methd(x) and A.class_methd(y) is True
        ):  # # фильтр пропускает значения соотв условию из class_methd
            self.x = x
            self.y = y
...
print(
    D.class_methd(o.multi_ply()),
    " D.class_methd в аргументе результат метода o.multi_ply()",
)
...
print(
    P.class_methd(s.multi_ply()),
    " D.class_methd в аргументе результат метода s.multi_ply()",
)
