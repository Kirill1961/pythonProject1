...
print(
    ...
    " linspace то же самое что и arange но num - задаёт кол-во случайных "
...
print(
    default_rng(5).random((2, 3)),
    " default_rng - Создание матрицы с item type float от"
...
print(
    ...
    " np.random.randint -  Создание random матрицы из целых случайных чисел."
...
print(
    ...
    " Создание random матрицы с 0 и 1, интервал от 0 - 1,size"
...
print(
    np.random.randint(2, size=(2, 2, 5)),
    " Создание randomматрицы с 0 и 1, интервал от 0 - 1,size"
    " - кол-во значений, dim - любая",
    "\n",
)
...
g = np.array([[[4, 0, 1], [8, 9, 4], [0, 5, 4]], [[2, 9, 6], [9, 3, 5], [6, 1, 0]]])
...
for edge in edges:
    start, end = edge
    adjacency_matrix[start - 1][
        end - 1
    ] = 1  # Мы вычитаем 1, так как индексы в Python начинаются с 0
...
print(
    np.array([[(i, j) for i in a] for j in a]), "передружили i vs j"
)  # передружили i vs j
