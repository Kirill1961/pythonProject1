...
def majority_vote(
    labels,
):  # Подаём из knn_classify() - метки labels отсортированные по возрастанию
    ...
    print(
        winner, winner_count, " Победитель из", num_winners, " претиндентов"
...
def create_labled_points(number_points):
    ...
    #     print(' ')
    result = [
        i for i in zip(random_points, lables_for_points)
    ]  # создаём пары ТОЧКА - МЕТКА, список кортежей
    # print(result, ' result >>>>>>>>>>>>>>>>>>>>>')
    print("")
...
create_labled_points(
    4
), " create_labled_points"  # Задаём точки и метки в заданом количестве 10, метки у = х**2
...
def input_new_points(x):
    ...
    for result_new_point in [
        i for i in zip(random_points, lables_for_new_points)
    ]:  # метка и точка, список кортежей
...
def knn_classify(k, labled_points, new_points):
    ...
    by_distanse = sorted(
        labled_points,
        key=lambda labled_point_for_dist: math.dist(labled_point_for_dist, new_points),
    )

    print(
        by_distanse,
        "Отсортировали по удалённости от всех labled_points до новой new_points",
    )
    k_nearset_labels = [label for _, label in by_distanse[:k]]
    print(" ")
    print(
        k_nearset_labels,
        " k_nearset_labels, вывели три ближайших labled_points до новой new_points",
    )
