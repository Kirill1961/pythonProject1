...
while a < len(users):
    ...
    print(
        fr_of_fr_id_b(users[a - 1]),
        " -  кол-во друзей у друзей данного user  " + f"{a - 1}",
    )
...
print(
    not_the_same(users[0]["friends"], users[3]["friends"]),
    "           если id разные то пользователи неодинаковые",
)
...
print(
    data_scien_hwo_like("Hadoop"),
    "                <- какому id нравится данное увлечение ",
)
...
functwo = partial(
    func, c=1000
)  # на основе ф-ции func(a, b, c=8  создаём ф-ю basetwo с доп аргументом
