...
def count_words(training_set):  # # training_set - это train_data
    ...
    for message, is_spam in training_set:
        for word in tokenize(message):
            counts[word][
                0 if is_spam else 1
            ] += 1  # # если is_spam == True или False то +1 к value[0} или value[1}
...
def words_probablities(counts, total_spam, total_non_spam, k=0.5):
    ...
    ]  # # возвращаем список кортежей слово с вероятностями спам/неспам
...
def spam_probability(
    word_probs, message
):  # # word_probs - результат обучения, message - это test_data
    message_words = tokenize(message)
    # print(message_words,  "TOKENIZEEEEEEEEEEEEEEEEEEEEEEEEEE")

    log_prob_if_spam = log_prob_if_not_spam = (
        0.0  # #log - понимаем как регистрация или запись
    )
    for word, prob_if_spam, prob_if_not_spam in word_probs:
        # print("\t" * 4,word, prob_if_spam, prob_if_not_spam, " word_probs 1 ")
        if (
            word in message_words
        ):  # # если из test_data слово в сообщении то логируем его в переменную
            ...
            log_prob_if_spam += math.log(
                1 - prob_if_spam
            )  # # если из tokenize слова нет в сообщении
            log_prob_if_not_spam += math.log(
                1 - prob_if_not_spam
            )  # # то уменьшаем переменную

        # print(log_prob_if_spam, " spam", log_prob_if_not_spam, " not_spam")

    prob_if_spam = math.exp(
        log_prob_if_spam
    )  # # math.exp() перевод из log-вероятности обратно в вероятность в %
    ...
    )  # # вероятность появления слова в спаме p = n / m
...
for fn in glob.glob(path):
    ...
    is_spam = (
        "ham" not in fn
    )  # # если в названии папки встречается "ham" то count_words в списке  [0 ,0] к value[1] +1
    # print("\t" * 7 , is_spam)
    with open(fn, "r") as file:
        for line in file:
            ...
            if line.startswith(("Subject")):
                ...
                data.append(
                    (subject, is_spam)
                )  # # маркируем сообщения спам/неспам - True/False
...
classifier.train(
    train_data
)  # # если этот метод не запустить, то не будет self.word_probs = [] и дальше не будет данных
...
classified = [
    ...
    )  # метод classify, получаем триплет (word,bool,prob)
    for subject, is_spam in test_data
]  # # создаём триплет < текст, спам/неспам, вероятность> на тест данных
print("\t" * 2, classified, " триплет classifieddddddddddddddd")
counts = Counter(
    (is_spam, spam_probability > 0.5) for _, is_spam, spam_probability in classified
)
...
spammist_hams = [filter(lambda row: not row[1], classified)][
    -1:
]  # # посл значение из фильтра мах вероятность по False
...
hammiest_spams = [filter(lambda row: row[1], classified)][
    :
]  # # первое значение из фильтра min вероятность по True
...
def p_spam_given_word(
    word_prob,
):  # # передаём переменную classifier.word_probs в ф-ю p_spam_given_word и сортируем
...
words = sorted(
    classifier.word_probs, key=p_spam_given_word
)  # # classifier.word_probs - переменная из класса
