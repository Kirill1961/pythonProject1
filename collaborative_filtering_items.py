...
#                    ]
users_interests = [
    ["Java", "R", "go", "scipy", "numpy", "MongoDB", "pandas", "data science"],
...
def make_user_interest_vector(user_interests):
    itrs = (
        list(1 if interest in user_interests else 0 for interest in unique_interests),
        "вектор интересов",
    )
    # print(itrs)
    return [
...
user_interest_matrix = [
    i for i in map(make_user_interest_vector, users_interests)
]  # матрица, строка - кол-во юзеров
...
def most_similar_users_to(interest_id):
    ...
    pairs = [
        (unique_interests[other_interest_id], similaraty)
        for other_interest_id, similaraty in enumerate(similarities)
        if interest_id != other_interest_id and similaraty > 0
...
def item_based_suggetions(user_id, include_current_interests=False):
    ...
    if include_current_interests:
        ...
        return [
            (suggetion, weight)
            for suggetion, weight in suggetions
