...
bigrams = zip(
    document, document[1:]
)  # zip составляет пары из 1-го и 2-го слов и сохраняем в bigrams
...
for prev, current in bigrams:
    transition[prev].append(
        current
    )  # вставляем в dict (transition) ключи prev и значения current
...
def generate_using_bigrams():
    ...
    while (
        True
    ):  # Если поставить while True: - то цикл работает до выполнения условия if
        next_word_candidates = transition[current]
        current = random.choice(
            next_word_candidates
        )  # рандомно выбираем value д/result[] и key д/transition
        result.append(current)
        if current == ".":
            return " ".join(
                result
            )  # условие для остановки while, if в тексте встретится ОБ "."


print(generate_using_bigrams(), " BIGRAMM", "\n" "")

trigrams = zip(document, document[1:], document[2:])  # разбиваем слова на тройки
trigram_transitions = defaultdict(
    list
)  # словарь по умолчанию, values - тип list позволяет использовать методы списков
starts = []
for (
    ...
    trigram_transitions[(prev, current)].append(
        next
    )  # Заполняем словарь key - (prev, current) и value - (next)
...
def generate_using_trigrams():
    ...
    while True:
        ...
        next_word_candidates = trigram_transitions[
            (prev, current)
        ]  # вызываем словарь с ключом (prev, current)
        # и сохраняем в переменную next_word_candidates значение в виде списка

        next_word = random.choice(
            next_word_candidates
        )  # случайно слово из next_word_candidates

        prev, current = (
            current,
            next_word,
        )  # перемещаем ОБ из переменной current в prev и из next_word в current
        result.append(current)  #
        if current == ".":
            return " ".join(result)
