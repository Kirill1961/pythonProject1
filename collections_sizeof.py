...
counts = defaultdict(
    lambda: [0, 0]
)  # lambda возвращает в словарь counts - value [0, 0], инициализируем словарь
for j in range(2):
    for i in range(5):
        counts[j][
            0
        ] += 1  # раздельно суммируем value [0, 0] с индексом [0] и индексом [1]
        counts[i][1] += 1
...
g = defaultdict(
    list
)  # если указываем ТИП данных в скобках, то генерируемые значения будут иметь ИХ свойства
g[88].append(55)
print(g, " append(55)")
...
e.default_factory = lambda: [
    1.2,
    3,
]  # Переопределение value если ключ без значения, переопределяем с list на [1.2,3]
...
print(
    first == second, " //// OrderedDict сравниваем словари, но не порядок"
)  # сравниваем словари, но не порядок
...
print(
    order1 == order2, " //// OrderedDict сравниваем порядок словарей"
...
print(
    counter.most_common(3)[2][0], " //// most_common", "\n"
...
print(
    ...
    " создали список объектов Counter для каждого вложенного LIST",
)
for k in range(len(documents)):
    for doc, word in zip(documents[k], topic[k]):
        document_topic_counts[k][
            word
        ] += 1  # [k] - индекс ОБ Counter, [word] - ключ, value - количество тем в doc
