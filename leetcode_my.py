...
def add_Two_Numbers(l3, l4):
    ...
    list_one = [
        sum(l_o) for l_o in zip(l3, l4)
    ]  # создание исходного списка из 2х списков
    last_ind = len(list_one) - 1  # индекс последнего ОБ
    for ind in range(len(list_one[0:-1])):  # перебор коллекции ОБ кроме последнего
        if (
            list_one[ind] >= 10
        ):  # фильтр для переноса единицы если все ОБ кроме последнего > или = 10
            list_one[ind] = list_one[ind] - 10
            list_one[ind + 1] = list_one[ind + 1] + 1
        if (
            list_one[last_ind] >= 10
        ):  # фильтр для переноса единицы последнего ОБ если он > или = 10
