...
def different_guotient(f, x, h):  # в f ф-я 1-й переменной, подставляется square
    ...
    return (
        f(x + h) - f(x)
    ) / h  # h - это '▲ х', приращение на оси х, f(x + h) - f(x)) - это '▲ у'
...
def step(v, direction, step_size):
    ...
    return [
        v_i + step_size * direction_i for v_i, direction_i in zip(v, direction)
    ]  # direction х -0.01 изм напр градие
...
while True:
    ...
    if (
        distance(next_v[0], v[0]) < tolerance
    ):  # останов при достижении приемлемого уровня (схождения)
        break
