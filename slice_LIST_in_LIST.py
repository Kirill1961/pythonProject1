...
print(
    [i for i in filter(lambda row: row, f)][:2],
    " СРЕЗЫ все до 2 го индекса включительно",
)
...
print(
    c[sum_of_lists.index(min(sum_of_lists))],
    " Нахождение в LIST[LIST] списка с min суммой значений",
)
...
print(
    ...
    "Объединение вложенных списков [[1,2,3], [4,5,6]]",
    "\n",
)
...
print(
    [a[row : (row + 5)] for row in range(0, 25, 5)],
...
i = [5, 1, 6, 2, 4, 3]  #
print(
    [[p for p in i if p in k[j]] for j in range(len(k))],
    "Выбор из произвольного list значений" " находящихся в list[list], K-means",
    "\n",
)
