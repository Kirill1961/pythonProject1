...
print([i for i in filter(lambda row: row, f)][4:], " СРЕЗЫ все начиная с 4 го индекса")
print([i for i in filter(lambda row: row, f)][:2], " СРЕЗЫ все до 2 го индекса не включительно")
...
# 1-й вариант, Разбить список на пары или др кол-во, comprehension срезов
d = [-49, 0, -4, 15, -152, 5, -21, 8, -34, 13]
print([d[j:2 + j]for j in range(0, 10, 2)], " Разбить список на пары или др кол-во", "\n")

# 2-й вариант, Разбить список на пары, тройки или др кол-во
a = [30, 20, 50, 80, 70, 90, 4, 9, 1, 7, 5, 8]
m = 2  # пары, тройки и тд
p = len(a)  # Кол-во пар, троек и тд
print([a[i: i + m] for i in range(0, p, m)], "2-й вариант, Разбить список на пары, тройки или др кол-во", "\n")


# Вывести покоординатно симметричный список для перемножения
print([[d[i + j]for i in range(0, 9, 5)] for j in range(5)], "Вывести покоординатно симметричный список для перемножения", "\n")


# Покоординатное умножение
a = [0.6, 0.4, 0.8, 70, 40, 30]
h = int(len(a ) / 2)
first = a[: h]
second = a[h:]
print([first[i] * second[i] for i in range(h)], "Покоординатное умножение", "\n")
print([a[: h][i] * a[h:][i] for i in range(h)], "Покоординатное умножение без переменных всё в куче", "\n")