...
print(
    direct_sample(),
    "direct_sample, PROB значений 1-го и суммы 1-го и 2-го кубиков",
    "\n",
)


def random_y_given_x(
    x,
):  # PROB Y при известном X - это условное распределение Р(y) = Р(y | x) ????

    return x, x + roll_a_die()


print(
    [random_y_given_x(x) for x in range(1, 7)],
    "совместное условное распределение" " условно зависимых Y и X при X = от 1 до 6",
    "\n",
)
...
def random_x_given_y(
    y,
):  # PROB X при известном Y - это условное распределение Р(х) = Р(х | y) ????
    if y <= 7:
...
print(
    [random_x_given_y(y) for y in range(2, 13)],
    "совместное условное распределение " "условно зависимых X и Y при Y = от 2 до 12",
    "\n",
)
...
def gibbs_sample(
    num_iter,
):  # безусловное распределение Х и У безусловно зависимых Х и У
    x, y = 6, 10
...
def compare_distribution(
    num_samples,
):  # Его вар-т заполнения dict для сравнения распр-ия Гиббса и прямого распр-ия
    counts = defaultdict(
        lambda: [1, 1]
    )  # lambda возвращает в словарь counts - value [0, 0] но лучше [1, 1]
...
def compare_distribution_my(
    num_samples,
):  # Мой вар-т заполнения dict для сравнения распр-ия Гиббса и прямого распр-ия
    counts_ = {}  # без defaultdict и lambda задаём пустой словарь
    for _ in range(num_samples):
        counts_[gibbs_sample(100)] = [
            direct_sample()
        ]  # ключ это распр-ие Гиббса, величина это прямое распределение
